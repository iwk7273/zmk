...
K_MUTEX_DEFINE(zmk_sensor_keymap_mutex);
...
int zmk_keymap_get_layer_sensor_binding_at_idx(zmk_keymap_layer_id_t layer_id,
    ...
    *binding = zmk_sensor_keymap[layer_id][sensor_idx];
...
#if IS_ENABLED(CONFIG_ZMK_KEYMAP_SETTINGS_STORAGE)
static int init_sensor_direction_bindings_from_stock(zmk_keymap_layer_id_t layer_id,
                                                     uint16_t sensor_idx) {
...
#if IS_ENABLED(CONFIG_ZMK_KEYMAP_SETTINGS_STORAGE)
        // The direction wrapper does not contain the bindings it dispatches to. This state can
        // occur while restoring settings when keymap/s exists but one or both keymap/sd entries
        // are missing. Seed from the stock sensor binding instead of manufacturing invalid #0
        // bindings for the missing directions.
        return init_sensor_direction_bindings_from_stock(layer_id, sensor_idx);
#else
        return -ENOTSUP;
#endif
    }
...
#if ZMK_KEYMAP_HAS_SENSORS
        k_mutex_lock(&zmk_sensor_keymap_mutex, K_FOREVER);
        uint8_t *sensor_pending = zmk_sensor_pending_changes[l];
...
static int save_binding_setting(const char *setting_name,
    ...
    size_t len = behavior_binding_setting_len(&binding_setting);
    return settings_save_one(setting_name, &binding_setting, len);
}
    ...
    for (int l = 0; l < ZMK_KEYMAP_LAYERS_LEN; l++) {
        ...
        for (int kp = 0; kp < ZMK_KEYMAP_LEN; kp++) {
            if (pending[kp / 8] & BIT(kp % 8)) {
                ...
                int ret = save_binding_setting(setting_name, binding);
                if (ret < 0) {
...
static int save_sensor_bindings(void) {
    for (int l = 0; l < ZMK_KEYMAP_LAYERS_LEN; l++) {
        uint8_t *pending = zmk_sensor_pending_changes[l];
        ...
        for (int s = 0; s < ZMK_KEYMAP_SENSORS_LEN; s++) {
            ...
            if (pending[s / 8] & BIT(s % 8)) {
                ...
                k_mutex_lock(&zmk_sensor_keymap_mutex, K_FOREVER);
                if (memcmp(&zmk_sensor_keymap[l][s], &binding, sizeof(binding)) == 0) {
...
struct keymap_changed_bindings {
...
static int keymap_track_changed_bindings(const char *key, size_t len, settings_read_cb read_cb,
    ...
    const char *next;
    struct keymap_changed_bindings *state = param;
    ...
    if (settings_name_steq(key, "l", &next) && next) {
        ...
        if (layer >= ZMK_KEYMAP_LAYERS_LEN || key_position >= ZMK_KEYMAP_LEN) {
            LOG_WRN("Invalid keymap setting location: layer %d key position %d", layer,
                    key_position);
            return -EINVAL;
        }

        WRITE_BIT(state->layer[layer][key_position / 8], key_position % 8, 1);
    }
#if ZMK_KEYMAP_HAS_SENSORS
        ...
        if (layer >= ZMK_KEYMAP_LAYERS_LEN || sensor_idx >= ZMK_KEYMAP_SENSORS_LEN) {
            LOG_WRN("Invalid sensor setting location: layer %lu sensor %lu", layer, sensor_idx);
            return -EINVAL;
        }
...
int zmk_keymap_reset_settings(void) {
    ...
    struct keymap_changed_bindings changes = {0};

    settings_load_subtree_direct("keymap", keymap_track_changed_bindings, &changes);
    ...
    for (int l = 0; l < ZMK_KEYMAP_LAYERS_LEN; l++) {
        ...
        uint8_t *layer_changes = changes.layer[l];
...
#if ZMK_KEYMAP_HAS_SENSORS
        uint8_t *sensor_changes = changes.sensor[l];
...
static int keymap_handle_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg) {
    ...
    if (settings_name_steq(name, "l_n", &next) && next) {
        ...
        if (layer >= ZMK_KEYMAP_LAYERS_LEN) {
            LOG_WRN("Layer %lu is larger than max of %d", layer, ZMK_KEYMAP_LAYERS_LEN);
            return -EINVAL;
        }

        if (sensor_idx >= ZMK_KEYMAP_SENSORS_LEN) {
            LOG_WRN("Sensor index %lu is larger than max of %d", sensor_idx,
                    ZMK_KEYMAP_SENSORS_LEN);
            return -EINVAL;
        }
        ...
        int err = read_cb(cb_arg, &binding_setting, len);
        if (err <= 0) {
            LOG_ERR("Failed to handle keymap sensor binding from settings (err %d)", err);
            return err;
        }
        ...
        if (!name) {
            LOG_WRN("Loaded sensor device %d from settings but no device found by that local ID",
                    binding_setting.behavior_local_id);
        }
        ...
        if (!zmk_sensor_direction_keymap_overrides[layer][sensor_idx]) {
            // Direction settings are self-contained. Seed any missing sibling from the stock
            // sensor binding so settings load does not depend on whether keymap/s or keymap/sd is
            // visited first by the backend.
            int ret = init_sensor_direction_bindings_from_stock(layer, sensor_idx);
            if (ret < 0) {
                k_mutex_unlock(&zmk_sensor_keymap_mutex);
                return ret;
            }
        ...
        zmk_sensor_direction_loaded_settings[layer][sensor_idx] |= BIT(param);
...
static int keymap_handle_commit(void) {
...
#if ZMK_KEYMAP_HAS_SENSOR_DIRECTION_BINDINGS
            // Behavior local IDs may not be resolved while keymap/s and keymap/sd settings are
            // visited. Defer this decision until commit so a temporarily NULL behavior_dev does
            // not erase direction settings that were already restored.
    ...
    for (int l = 0; l < ZMK_KEYMAP_LAYERS_LEN; l++) {
        for (int s = 0; s < ZMK_KEYMAP_SENSORS_LEN; s++) {
            if (!is_sensor_direction_bindings_behavior(zmk_sensor_keymap[l][s].behavior_dev)) {
                continue;
            }
